...
print(calculate_mode([34, 23, 56, 43, 56, 2,
      378, 54, 23, 34, 12, 21, 23, 45, 55]))
...
print(calculate_range([34, 23, 56, 43, 56, 2,
      378, 54, 23, 34, 12, 21, 23, 45, 55]))
...
def is_prime(arg):
    for i in range(2, arg - 1):
        if arg % i == 0:
            return print('{} No es un numero primo'.format(str(arg)))

    return print('{} Es un numero primo'.format(str(arg)))
...
def is_unique(arg):
    for i in range(0, len(arg)):
        for x in range(i+1, len(arg)):
            if arg[i] == arg[x]:
                return print('El Arreglo no tiene valores únicos')

    return print('La lista tiene solo valores unicos')


is_unique([1, 2, 3, 4, 5, 2])
# Write a function which checks if all the items of the list are of the same data type.


def is_unique_type(arg):
    for i in range(0, len(arg)):
        if type(arg[i]) != type(arg[0]):
            return print('El Arreglo no tiene solo un  tipo de datos')

    return print('El Arreglo tiene solo un  tipo de datos')


is_unique_type([1, 2, 3, 4, 5, 2, 'h'])


# Write a function which check if provided variable is a valid python variable

def check_var(args):
    matriz = [type('h'), type(3), type(3.4), type(True),
              type([1, 2, 3]), type({"uno": "uno"}), type((1, 2)), type(zip([1, 2], [3, 4])), type({1, 2}), type(1+3j)]

    for i in matriz:
        if i == type(args):
            return print('La variable {} es del tipo {}'.format(str(args), str(i)))

    print('La variable no es del tipo python ')
...
def most_spoken_languages():
    ...
    for pais in data:
        for lan in pais['languages']:
            lenguas.append(lan)

    set_lenguas = set(lenguas)
    print('Total de lenguas habladas es :', len(set_lenguas))
# 2. Find the ten most spoken languages from the data
    languajes = {}

    for lan in list(set_lenguas):
        languajes[lan] = 0
# print(languajes)
...
# print(languajes)
    ...
    sorted_lenguas = dict(
        sorted(languajes.items(), key=lambda item: item[1], reverse=True))
    print(sorted_lenguas)

    ten_leguas = {}
    limit = 0

    for lan in sorted_lenguas:
        print(sorted_lenguas[lan])
        ten_leguas[lan] = sorted_lenguas[lan]
        limit += 1
        if limit > 9:
            break
...
def most_ledged_languages():
    population = {}

    for lan in data:
        population[lan['name']] = lan['population']

    sorted_population = dict(
        sorted(population.items(), key=lambda item: item[1], reverse=True))
    ten_population = {}
    limit = 0

    for lan in sorted_population:
        ten_population[lan] = sorted_population[lan]
        limit += 1
        if limit > 9:
            break
